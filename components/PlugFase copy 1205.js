...
const PlugFase = () => {
  ...
  const inputGroupRef = useRef(null); // <<< NOVO: container do autocomplete

  useEffect(() => {
    if (confirmOpen) setTimeout(() => confirmBtnRef.current?.focus(), 0);
  }, [confirmOpen]);

  // Clique fora: usa o container ref (sem closest)
  useEffect(() => {
  ...
  const handleAddCPS = async (forceName) => {
    ...
    try {
      ...
      // mesmo em caso de retorno false do contexto, limpamos a UI pra não travar
      setSearchText('');
      setSuggestions([]);
      setSuggestionsOpen(false);
      setHighlightIndex(-1);
  ...
  return (
    <div className="component-container plug-fase">
          ...
          <ul className="suggestions-list" role="listbox">
            {suggestions.map((name, idx) => (
              <li
                ...
                onClick={() => handleAddCPS(name)}      // clique já adiciona
      ...
      {selectedName && (
        ...
        >
          <span>
            Selecionado para remover: <strong>{selectedName}</strong>
          </span>
          <button
            type="button"
            onClick={() => {
          ...
          {addedCPS.length > 0 ? (
            addedCPS.map((cps) => {
              ...
              return (
                <li
                  key={cps.id}
                  className={`cps-item-plug ${isSelected ? 'selected' : ''}`}
                  onClick={() => handleSelectFromList(name)}
                  aria-selected={isSelected}
                  title={isSelected ? 'Selecionado para remover' : 'Clique para selecionar para remoção'}
                  style={{
                    cursor: 'pointer',
                    border: isSelected ? '1px solid #4b9cff' : '1px solid transparent',
                    background: isSelected ? 'rgba(75,156,255,0.08)' : 'transparent',
                    borderRadius: 8,
                    padding: '6px 8px',
                    marginBottom: 6
                  }}
